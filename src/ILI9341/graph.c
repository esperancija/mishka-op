...
uint16_t iconPixels[ICON_H*ICON_W];
uint32_t i,j;
...
for (j=0; j<ICON_H; j++){
	for (i=0; i<ICON_W; i++){
		if (icon[j/8+i*ICON_H/8] & (1<<(j%8)))
				iconPixels[j*ICON_W + i] = color;


		//iconPixels[j*ICON_W + i] = i%2?WHITE:BLACK;
	}
}
LCD_setAddressWindowToWrite(x, y, x+ICON_W-1, y+ICON_H-1);
dmaSendData16(iconPixels, ICON_W*ICON_H);
}
